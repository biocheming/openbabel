...
int main(int argc,char **argv)
{
  ...
  if (argc < 2) {
    ...
    for (int i = 1; i < argc; i++) {
      ...
      // set occupancy constraints
      if ((option == "-fc") && (argc > (i+1))) {
        c_occu = atof(argv[i+1]);
        ifile += 2;
      }
      // set constraint factor scale
      if ((option == "-fsc") && (argc > (i+1))) {
          c_scale = atof(argv[i+1]);
          if(c_scale <= 0) c_scale = 1e-64;
          ifile += 2;
      }
      // set fixed atom
        if ((option == "-fix") && (argc > (i+1))) {
            c_fixed = atof(argv[i+1]);
            ifile += 2;
        }
  ...
  for (c=1;;c++) {
    ...
    if(opt_h_only){
        for (auto ii = mol.BeginAtoms(); ii != mol.EndAtoms(); ii++) {
            if (!(*ii)->IsElement(1)) constraints.AddAtomConstraint((*ii)->GetIdx());
        }
    }
    
    double i_occupancy;
    double constrained_factor;
    for(auto ii = mol.BeginAtoms(); ii != mol.EndAtoms(); ii++){
      if ((*ii)->HasData("_atom_site_occupancy")){
          ...
          if (i_occupancy != c_occu) {
              ...
              else constraints.AddAtomConstraint((*ii)->GetIdx(), constrained_factor);
          }
      }
    }
